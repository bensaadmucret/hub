...
final class ContactController extends AbstractController
{
    ...
    public function contact(Request $request): Response
    {
        if ($request->isMethod('POST')) {
            // Rate limiting: 5 req/min per IP (configured in rate_limiter.yaml)
            $clientKey = $request->getClientIp() ?? 'anon';
            $limiter = $this->contactFormLimiter->create($clientKey);
            ...
            if (!$limit->isAccepted()) {
                ...
                $url = is_string($referer) && $referer !== '' ? $referer : $this->generateUrl('app_home');
                return $this->redirect($url);
            }
            // Vérification du token CSRF
            $tokenRaw = $request->request->get('_token');
            $tokenStr = is_string($tokenRaw) ? $tokenRaw : null;
            if (!$this->csrfTokenManager->isTokenValid(new CsrfToken('contact_form', $tokenStr))) {
                if ($request->isXmlHttpRequest()) {
            ...
            try {
                ...
                $referer = $request->headers->get('referer');
                $baseUrl = is_string($referer) && $referer !== '' ? $referer : $this->generateUrl('app_home');
                return $this->redirect($baseUrl . '#contact');
            } catch (\Exception $e) {
